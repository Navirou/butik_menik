...
<div class="bm-page-header d-flex justify-content-between align-items-start gap-3">
  ...
  <a href="?action=mark_read" class="btn-ivory" style="font-size:.85rem"><i class="bi bi-check2-all me-1"></i>Tandai Semua Dibaca</a>
  <?php endif; ?>
</div>
...
<div class="bm-card">
  ...
  <div class="d-flex gap-3 px-4 py-3 <?= $n['is_read']?'':'bg-ivory' ?>" style="border-bottom:1px solid var(--border)">
    ...
    <?php if (!$n['is_read']): ?>
    <a href="?action=mark_read&id=<?= $n['id'] ?>" style="flex-shrink:0;color:var(--navy);font-size:.75rem;font-weight:600;white-space:nowrap;text-decoration:none;align-self:center">Baca</a>
    <?php endif; ?>
  </div>
  <?php endforeach; ?>
  <?php endif; ?>
