...
<div class="bm-page-header d-flex justify-content-between align-items-start gap-3">
  ...
  <a href="?action=mark_read" class="btn-ivory" style="font-size:.85rem">
...
<div class="bm-alert bm-alert-success bm-alert-auto-dismiss">
  <i class="bi bi-check-circle-fill"></i>Semua notifikasi ditandai dibaca.
</div>
<?php endif; ?>

<div class="d-flex gap-2 mb-4 flex-wrap">
  <?php foreach (['all' => 'Semua', 'unread' => 'Belum Dibaca'] as $k => $l): ?>
  <a href="?filter=<?= $k ?>" class="bm-badge text-decoration-none"
     style="font-size:.82rem;padding:.5em 1.1em;<?= $notifFilter === $k
...
<div class="bm-card">
  ...
  <div class="bm-card-body text-center py-5 text-muted">
    ...
    <p>Tidak ada notifikasi<?= $notifFilter === 'unread' ? ' belum dibaca' : '' ?>.</p>
  </div>
  <?php else: ?>
  <?php foreach ($notifications as $n):
    [$ico, $bg, $ic] = $notifIcons[$n['type']] ?? ['bi-bell-fill', '#f3f4f6', '#6b7280'];
  ?>
  <div class="d-flex gap-3 px-4 py-3 <?= $n['is_read'] ? '' : '' ?>"
       style="border-bottom:1px solid var(--border);background:<?= $n['is_read'] ? '#fff' : 'var(--ivory)' ?>">
    ...
    <div style="flex:1;min-width:0">
      <div style="font-weight:<?= $n['is_read'] ? '500' : '700' ?>;font-size:.9rem;color:var(--text-main)">
        <?= htmlspecialchars($n['title']) ?>
        <?php if (!$n['is_read']): ?>
        <span class="bm-badge bm-badge-warning ms-1" style="font-size:.62rem">Baru</span>
        <?php endif; ?>
      </div>
      <div style="font-size:.82rem;color:var(--text-muted);margin:.2rem 0"><?= htmlspecialchars($n['body']) ?></div>
      <div style="font-size:.74rem;color:#adb5bd">
        <i class="bi bi-clock me-1"></i><?= date('d M Y, H:i', strtotime($n['created_at'])) ?>
      </div>
    </div>
    <?php if (!$n['is_read']): ?>
    <a href="?action=mark_read&id=<?= $n['id'] ?>"
       style="flex-shrink:0;color:var(--navy);font-size:.75rem;font-weight:600;white-space:nowrap;text-decoration:none;align-self:center">
      Baca
    </a>
    <?php endif; ?>
  </div>
